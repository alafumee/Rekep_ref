...
def resize(img: Image, target_res, resize=True, to_pil=True, edge=False):
    ...
    if not edge:
        ...
        if original_height <= original_width:
            # print(img.size,end="  image size before resize \n")
            if resize:
                img = img.resize((target_res, int(np.around(target_res * original_height / original_width))), Image.Resampling.LANCZOS)
                # print(img.size,end="  image size after resize \n")
            width, height = img.size
            # print(img,end="  image pixel value \n")
            img = np.asarray(img)
            # print(img[0][0],end="  image pixel value \n")
            # print(img.shape,end="  image shape after resize \n")
